...
def ae_beam_search(models, session, x_in, x_mask_in, beam_size):
    # x_in is a numpy array with shape (factors, seqLen, batch)
    # x_mask is a numpy array with shape (seqLen, batch)
    x_in = numpy.repeat(x_in, repeats=beam_size, axis=-1)
    x_mask_in = numpy.repeat(x_mask_in, repeats=beam_size, axis=-1)
    ...
    for model in models:
        ...
        feeds[model.x] = x_in
        feeds[model.x_mask] = x_mask_in
    beam_ys, parents, cost, ae_beam_ys, ae_parents, ae_cost = construct_beam_search_functions(models, beam_size)
    beam_ys_out, parents_out, cost_out, ae_beam_ys_out, ae_parents_out, ae_cost_out = session.run(
                                                    [beam_ys, parents, cost, ae_beam_ys, ae_parents, ae_cost],
                                                    feed_dict=feeds)
    return [ae_reconstruct_hypotheses(beam_ys_out, parents_out, cost_out,beam_size), ae_reconstruct_hypotheses(ae_beam_ys_out, ae_parents_out, ae_cost_out, beam_size)]
...
def construct_beam_search_functions(models, beam_size):
    ...
    ae_decoder = models[0].ae_decoder

    batch_size = tf.shape(decoder.init_state)[0]
    ae_batch_size = tf.shape(ae_decoder.init_state)[0]

    embedding_size = decoder.embedding_size
    ae_embedding_size = ae_decoder.embedding_size

    translation_maxlen = decoder.translation_maxlen
    ae_translation_maxlen = ae_decoder.translation_maxlen

    target_vocab_size = decoder.target_vocab_size
    ae_target_vocab_size = ae_decoder.target_vocab_size

    high_depth = 0 if decoder.high_gru_stack == None \
                   else len(decoder.high_gru_stack.grus)

    ae_high_depth = 0 if ae_decoder.high_gru_stack == None \
                   else len(ae_decoder.high_gru_stack.grus)
    ...
    ae_i = tf.constant(0)
    ae_init_ys = -tf.ones(dtype=tf.int32, shape=[ae_batch_size])
    ae_init_embs = [tf.zeros(dtype=tf.float32, shape=[ae_batch_size,ae_embedding_size])] * len(models)
    ...
    ae_init_cost = [0.] + [f_min]*(beam_size-1) # to force first top k are from first hypo only
    ae_init_cost = tf.constant(ae_init_cost, dtype=tf.float32)
    ae_init_cost = tf.tile(ae_init_cost, multiples=[ae_batch_size/beam_size])
    ae_ys_array = tf.TensorArray(
                dtype=tf.int32,
                size=ae_translation_maxlen,
                clear_after_read=True,
                name='ae_y_sampled_array')
    ae_p_array = tf.TensorArray(
                dtype=tf.int32,
                size=ae_translation_maxlen,
                clear_after_read=True,
                name='ae_parent_idx_array')
    ae_init_base_states = [m.ae_decoder.init_state for m in models]
    ae_init_high_states = [[m.ae_decoder.init_state] * ae_high_depth for m in models]
    ae_init_loop_vars = [ae_i, ae_init_base_states, ae_init_high_states, ae_init_ys, ae_init_embs,
                      ae_init_cost, ae_ys_array, ae_p_array]

    # Prepare cost matrix for completed sentences -> Prob(EOS) = 1 and Prob(x) = 0
    ae_eos_log_probs = tf.constant(
                        [[0.] + ([f_min]*(ae_target_vocab_size - 1))],
                        dtype=tf.float32)
    ae_eos_log_probs = tf.tile(ae_eos_log_probs, multiples=[ae_batch_size,1])
    ...
    def ae_cond(i, prev_base_states, prev_high_states, prev_ys, prev_embs, cost, ys_array, p_array):
        return tf.logical_and(
                tf.less(i, ae_translation_maxlen),
                tf.reduce_any(tf.not_equal(prev_ys, 0)))
    ...
    def ae_body(i, prev_base_states, prev_high_states, prev_ys, prev_embs, cost, ys_array, p_array):
        # get predictions from all models and sum the log probs
        sum_log_probs = None
        base_states = [None] * len(models)
        high_states = [None] * len(models)
        for j in range(len(models)):
            d = models[j].ae_decoder
            states1 = d.grustep1.forward(prev_base_states[j], prev_embs[j])
            att_ctx = d.attstep.forward(states1)
            base_states[j] = d.grustep2.forward(states1, att_ctx)
            if d.high_gru_stack == None:
        ...
        # Need to make sure only EOS is selected because a completed sentence might
        # kill ongoing sentences
        sum_log_probs = tf.where(tf.equal(prev_ys, 0), ae_eos_log_probs, sum_log_probs)

        all_costs = sum_log_probs + tf.expand_dims(cost, axis=1) # TODO: you might be getting NaNs here since -inf is in log_probs

        all_costs = tf.reshape(all_costs,
                               shape=[-1, ae_target_vocab_size * beam_size])
        values, indices = tf.nn.top_k(all_costs, k=beam_size) #the sorted option is by default True, is this needed?
        new_cost = tf.reshape(values, shape=[ae_batch_size])
        offsets = tf.range(
                    start = 0,
                    delta = beam_size,
                    limit = ae_batch_size,
                    dtype=tf.int32)
        offsets = tf.expand_dims(offsets, axis=1)
        survivor_idxs = (indices/ae_target_vocab_size) + offsets
        new_ys = indices % ae_target_vocab_size
        survivor_idxs = tf.reshape(survivor_idxs, shape=[ae_batch_size])
        new_ys = tf.reshape(new_ys, shape=[ae_batch_size])
        new_embs = [m.ae_decoder.y_emb_layer.forward(new_ys) for m in models]
        new_base_states = [tf.gather(s, indices=survivor_idxs) for s in base_states]
        new_high_states = [[tf.gather(s, indices=survivor_idxs) for s in states] for states in high_states]
        new_cost = tf.where(tf.equal(new_ys, 0), tf.abs(new_cost), new_cost)
    ...
    ae_final_loop_vars = tf.while_loop(
                        cond=ae_cond,
                        body=ae_body,
                        loop_vars=ae_init_loop_vars,
                        back_prop=False)
    ae_i, _, _, _, _, ae_cost, ae_ys_array, ae_p_array = ae_final_loop_vars
    ...
    ae_indices = tf.range(0, ae_i)
    ae_sampled_ys = ae_ys_array.gather(ae_indices)
    ae_parents = ae_p_array.gather(ae_indices)
    ae_cost = tf.abs(ae_cost) #to get negative-log-likelihood

    return sampled_ys, parents, cost, ae_sampled_ys, ae_parents, ae_cost
...
def ae_reconstruct_hypotheses(ys, parents, cost, beam_size):
    #ys.shape = parents.shape = (seqLen, beam_size x batch_size)
    # output: hypothesis list with shape (batch_size, beam_size, (sequence, cost))
