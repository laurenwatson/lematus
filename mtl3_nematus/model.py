...
class AEDecoder(object):
    def __init__(self, config, context, x_mask, dropout_target,
        ...
        with tf.name_scope("initial_state_constructor"):
            ...
            self.embedding_size = config.embedding_size
            self.state_size = config.state_size
            self.target_vocab_size = config.ae_target_vocab_size


        with tf.name_scope("embedding"):
            self.y_emb_layer = EmbeddingLayer(
                                vocabulary_size=config.ae_target_vocab_size,
                                embedding_size=config.embedding_size)
...
class POSDecoder(object):
    def __init__(self, config, context, x_mask, dropout_target,
        ...
        with tf.name_scope("initial_state_constructor"):
            ...
            self.embedding_size = config.embedding_size
            self.state_size = config.state_size
            self.target_vocab_size = config.pos_target_vocab_size


        with tf.name_scope("embedding"):
            self.y_emb_layer = EmbeddingLayer(
                                vocabulary_size=config.pos_target_vocab_size,
                                embedding_size=config.embedding_size)
        ...
        with tf.name_scope("next_word_predictor"):
            ...
            if config.tie_decoder_embeddings:
                W = self.y_emb_layer.get_embeddings()
                W = tf.transpose(W)
            self.predictor = POSPredictor(config, batch_size, dropout_embedding,
                                       dropout_hidden, hidden_to_logits_W=W)
...
class AEPredictor(object):
    def __init__(self, config, batch_size, dropout_embedding, dropout_hidden, hidden_to_logits_W=None):
        ...
        with tf.name_scope("hidden_to_logits"):
            self.hidden_to_logits = FeedForwardLayer(
                            in_size=config.embedding_size,
                            out_size=config.ae_target_vocab_size,
                            batch_size=batch_size,
                            non_linearity=lambda y: y,
                            W=hidden_to_logits_W,
                            dropout_input=dropout_embedding)
...
class POSPredictor(object):
    def __init__(self, config, batch_size, dropout_embedding, dropout_hidden, hidden_to_logits_W=None):
        ...
        with tf.name_scope("hidden_to_logits"):
            self.hidden_to_logits = FeedForwardLayer(
                            in_size=config.embedding_size,
                            out_size=config.pos_target_vocab_size,
                            batch_size=batch_size,
                            non_linearity=lambda y: y,
                            W=hidden_to_logits_W,
                            dropout_input=dropout_embedding)
...
class AEModel(object):
    def __init__(self, config):
        ...
        self.y_mask = tf.placeholder(
                        ...
                        name='y_mask',
                        shape=(seqLen, batch_size))
        self.ae_y = tf.placeholder(
                    dtype=tf.int32,
                    name='ae_y',
                    shape=(seqLen, batch_size))
        self.ae_y_mask = tf.placeholder(
                        dtype=tf.float32,
                        name='ae_y_mask',
                        shape=(seqLen, batch_size))
        self.pos_y = tf.placeholder(
                    dtype=tf.int32,
                    name='pos_y',
                    shape=(seqLen, batch_size))
        self.pos_y_mask = tf.placeholder(
                        dtype=tf.float32,
                        name='pos_y_mask',
                        shape=(seqLen, batch_size))
        self.training = tf.placeholder_with_default(
        ...
        # These probabilistically zero-out all embedding values for individual
        # words.
        dropout_source, dropout_target, dropout_ae_target, dropout_pos_target = None, None, None, None
        if config.use_dropout and config.dropout_source > 0.0:
            def dropout_source(x):
                return tf.layers.dropout(
                    x, noise_shape=(tf.shape(x)[0], tf.shape(x)[1], 1),
        ...
        if config.use_dropout and config.dropout_target > 0.0:
            def dropout_target(y):
                return tf.layers.dropout(
                    y, noise_shape=(tf.shape(y)[0], tf.shape(y)[1], 1),
                    rate=config.dropout_target, training=self.training)
        if config.use_dropout and config.dropout_ae_target> 0.0:
            def dropout_ae_target(ae_y):
                return tf.layers.dropout(
                    ae_y, noise_shape=(tf.shape(y)[0], tf.shape(y)[1], 1),
                    rate=config.dropout_ae_target, training=self.training)
        if config.use_dropout and config.dropout_pos_target> 0.0:
            def dropout_pos_target(pos_y):
                return tf.layers.dropout(
                    pos_y, noise_shape=(tf.shape(y)[0], tf.shape(y)[1], 1),
                    rate=config.dropout_pos_target, training=self.training)
        ...
        with tf.name_scope("decoder"):
            ...
            self.ae_decoder = AEDecoder(config, ctx, self.x_mask, dropout_ae_target,
                                   dropout_embedding, dropout_hidden)
            self.pos_decoder = POSDecoder(config, ctx, self.x_mask, dropout_pos_target,
                                   dropout_embedding, dropout_hidden)
            #what does score do?
            self.logits = self.decoder.score(self.y)
            #I think score just pushes it through the decoder
            self.ae_logits = self.ae_decoder.score(self.ae_y)
            self.pos_logits = self.pos_decoder.score(self.pos_y)
        ...
        with tf.name_scope("loss"):
            ...
            self.ae_loss_layer = Masked_cross_entropy_loss(self.ae_y, self.ae_y_mask)
            self.pos_loss_layer = Masked_cross_entropy_loss(self.pos_y, self.pos_y_mask)
            ...
            self.pos_loss_per_sentence = self.pos_loss_layer.forward(self.pos_logits)

            #gets mean loss over all sentences
            self.mean_loss = tf.reduce_mean(self.loss_per_sentence, keep_dims=False)
            self.ae_mean_loss = tf.reduce_mean(self.ae_loss_per_sentence, keep_dims=False)
            self.pos_mean_loss = tf.reduce_mean(self.pos_loss_per_sentence, keep_dims=False)

            #try just adding them?
            self.objective = alpha*self.mean_loss + ((1-alpha)/2)*self.ae_mean_loss + ((1-alpha)/2)*self.pos_mean_loss
    ...
    def get_score_inputs(self):
        return self.x, self.x_mask, self.y, self.y_mask, self.ae_y, self.ae_y_mask, self.pos_y, self.pos_y_mask,self.training
    ...
    def get_pos_loss(self):
        return self.pos_loss_per_sentence

    def get_mean_loss(self):
        return self.mean_loss

    def get_mean_ae_loss(self):
        return self.ae_mean_loss

    def get_mean_pos_loss(self):
        return self.pos_mean_loss
...
class AEModel_Alternate(object):
    def __init__(self, config):
        ...
        with tf.name_scope("loss"):
            ...
            #try just adding them?
            self.objective = self.mean_loss
            self.ae_objective = self.ae_mean_loss
            self.pos_objective = self.pos_mean_loss
        ...
        grad_ae_vars = self.optimizer.compute_gradients(self.ae_mean_loss)
        grads_ae, varss_ae = zip(*grad_ae_vars)
        clipped_ae_grads, global_ae_norm = tf.clip_by_global_norm(grads_ae, clip_norm=config.clip_c)
        # Might be interesting to see how the global norm changes over time, attach a summary?
        grad_ae_vars = zip(clipped_ae_grads, varss_ae)

        grad_pos_vars = self.optimizer.compute_gradients(self.pos_mean_loss)
        grads_pos, varss_pos = zip(*grad_pos_vars)
        clipped_pos_grads, global_pos_norm = tf.clip_by_global_norm(grads_pos, clip_norm=config.clip_c)
        # Might be interesting to see how the global norm changes over time, attach a summary?
        grad_pos_vars = zip(clipped_pos_grads, varss_pos)

        self.apply_grads = self.optimizer.apply_gradients(grad_vars, global_step=self.t)
        self.apply_ae_grads = self.optimizer.apply_gradients(grad_ae_vars, global_step=self.t)
        self.apply_pos_grads = self.optimizer.apply_gradients(grad_pos_vars, global_step=self.t)
    ...
    def get_ae_objective(self):
        return self.ae_objective

    def get_pos_objective(self):
        return self.pos_objective
    ...
    def get_apply_ae_grads(self):
        return self.apply_ae_grads

    def get_apply_pos_grads(self):
        return self.apply_pos_grads
