...
do
    for lang in "${languages[@]}"
    do
        targetDir=languages_10/${lang}-${N}-char-context
        mkdir -p ${targetDir}

        python3 format_for_lematus_10.py $UD_directory/UD_${lang}/dev ${lang} dev ${n}
        mv dev-* ${targetDir}/.

        python3 format_for_lematus_10.py $UD_directory/UD_${lang}/train ${lang} train ${n}
        mv train-* ${targetDir}/.

        python3 format_for_lematus_10.py $UD_directory/UD_${lang}/test ${lang} test ${n}
        mv test-* ${targetDir}/.
